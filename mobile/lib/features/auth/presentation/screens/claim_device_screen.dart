...
/// Bu cihaz hesabın active_device_id-si DEYİL. İstifadəçi OTP girib bu cihazı
/// yeni "yeganə aktiv cihaz" kimi qeyd edə bilər. Köhnə cihaz növbəti API
/// çağırışında 403 alacaq və avtomatik bu ekrana düşəcək (kim öncə claim
/// edirsə, o qalır).
...
class _ClaimDeviceScreenState extends ConsumerState<ClaimDeviceScreen> {
  ...
  Future<void> _bootstrap() async {
    // device-status yalnız label-i göstərmək üçündür. Backend Dio interceptor
    // X-Device-Id qoşur, lakin 403 device_mismatch artıq baş verib (deməli
    // bu endpoint guard-dan kənarda olmalı idi). Hələlik səssizcə davam edirik
    // və OTP göndərməyə keçirik.
    try {
      final dio = ref.read(dioProvider);
      final res = await dio.get('/auth/device-status');
      final data = res.data as Map<String, dynamic>?;
      if (mounted && data != null) {
        setState(() => _activeLabel = data['activeDeviceLabel'] as String?);
      }
    } catch (_) {/* səssiz */}
    if (!mounted) return;
  ...
  Future<void> _sendOtp({bool initial = false}) async {
    ...
    try {
      final dio = ref.read(dioProvider);
      await dio.post('/auth/send-device-otp');
      if (!mounted) return;
  ...
  Future<void> _claim() async {
    ...
    try {
      ...
      await dio.post('/auth/claim-device', data: {
        'code': code,
        'deviceId': deviceId,
        'deviceLabel': deviceLabel(),
      });
      if (!mounted) return;
      // Backend active_device_id-ni bu cihaza yenilədi. Bayrağı sıfırla,
      // profile cache-i yenidən fetch et və /home-a get.
  ...
  String _otpError(AppLocalizations l10n, DioException e) {
    ...
    if (code == 400) return l10n.signupOtpInvalid;
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          '${l10n.claimDeviceActiveLabel}: $_activeLabel',
